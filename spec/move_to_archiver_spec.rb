RSpec.describe Balboa::MoveToArchiver do
  let(:test_root_dir) { Dir.mktmpdir "spec_balboa_gem_" }

  let(:archiver) { Balboa::MoveToArchiver.new(map) }
  ...
  describe "#archive" do
    ...
    it "moves the files to the destination directory" do
      Dir.chdir(test_root_dir) do
        ...
        expect(File).to_not exist(File.join(test_root_dir, path_1))
        expect(File).to_not exist(File.join(test_root_dir, path_2))
      end
    end

    it "ignores files not in the map" do
      Dir.chdir(test_root_dir) do
